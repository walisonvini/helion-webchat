        ...
        <div class="md:flex w-full">
          ...
          <div class="w-full md:w-1/2 py-10 px-5 md:px-10">
            <div class="text-center mb-10">
              <h1 class="font-bold text-3xl text-gray-900">REGISTRE-SE</h1>

              <div class="p- mb-4 mt-4 text-md text-green-800 rounded-lg  dark:text-green-400 bg-green-100" role="alert"  v-show="success">
                <span class="font-medium">Registrado com sucesso!</span>
              </div>

              <div class="items-center flex p-4 mb-4 mt-4 text-md text-red-800 rounded-lg dark:text-red-400 bg-red-100" v-show="error">
                ...
                <div class="flex -mx-3">
                  <div class="w-1/2 px-3 mb-5">
                    ...
                    <div class="flex">
                      ...
                      <input type="text" class="w-full -ml-10 pl-10 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" :class="{ 'border-red-400': form.full_name.invalidFeedback }" placeholder="John" v-model="form.full_name.value">
                    </div>
                  </div>
                  <div class="w-1/2 px-3 mb-5">
                    <label for="" class="text-xs font-semibold px-1">Nome de usuario</label>
                    <div class="flex">
                        <div class="w-10 z-10 pl-1 text-center pointer-events-none flex items-center justify-center"><i class="mdi mdi-account-outline text-gray-400 text-lg"></i></div>
                        <input type="text" class="w-full -ml-10 pl-10 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" :class="{ 'border-red-400': form.username.invalidFeedback }" placeholder="Smith" v-model="form.username.value">
                    </div>
                  </div>
                </div>
                <div class="flex -mx-3">
                  <div class="w-full px-3 mb-5">
                    ...
                    <div class="flex">
                      ...
                      <input type="email" class="w-full -ml-10 pl-10 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" :class="{ 'border-red-400': form.email.invalidFeedback }" placeholder="johnsmith@example.com" v-model="form.email.value">
                    </div>
                  </div>
                </div>
                  <div class="flex -mx-3">
                    <div class="w-full px-3 mb-12">
                      ...
                      <div class="flex">
                        ...
                        <input type="password" class="w-full -ml-10 pl-10 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" :class="{ 'border-red-400': form.password.invalidFeedback }" placeholder="************" v-model="form.password.value">
                      </div>
                    </div>
                  </div>
                  <div class="flex -mx-3">
                    <div class="w-full px-3 mb-5">
                      <button type="submit" class="block w-full max-w-xs mx-auto bg-indigo-500 hover:bg-indigo-700 focus:bg-indigo-700 text-white rounded-lg px-3 py-3 font-semibold">REGISTRAR</button>
                    </div>
                  </div>
                </form>
          </div>
...
export default {
  setup() {
    ...
    async function submitForm() {
      ...
      form.username.invalidFeedback = false;
      form.password.invalidFeedback = false;
      ...
      try {
        ...
        if(response.status === 400) {
          ...
          error.value = "Preencha os campos obrigatorios";

          if ("username" in errors) {
            form.username.invalidFeedback = errors.username;
          } if ("password" in errors) {
            form.password.invalidFeedback = errors.password;
            if(errors.password != true)
              error.value = errors.password
          } if("email" in errors) {
            form.email.invalidFeedback = errors.email;
        ...
        if(response.status === 409) {
          ...
          if(message.includes("usuário")) {
            form.username.invalidFeedback = true;
          } else {
    ...
    return { form, submitForm, success, error }
  }
}
</script>
